...
{
  Scroom::Utils::Rectangle<double>     getRect() { return Scroom::Utils::Rectangle<int>(0, 0, 100, 100); }
  ...
  bool                                 isPropertyDefined(const std::string& name) { return PIPETTE_PROPERTY_NAME == name; }
  std::string                          getTitle() { return nullptr; }
  void                                 open(ViewInterface::WeakPtr){};
  void                                 close(ViewInterface::WeakPtr){};
  PipetteLayerOperations::PipetteColor getPixelAverages(Scroom::Utils::Rectangle<int>) { return {{"C", 1.0}}; }
};
...
BOOST_AUTO_TEST_CASE(pipette_value_display)
{
  ...
  handler->computeValues(ViewInterface::Ptr(new DummyView), Scroom::Utils::Rectangle<int>(-10, -11, 20, 22));
