...
namespace Scroom::Utils
{
  ...
  template <typename T>
  [[nodiscard]] std::optional<T> try_get(const Context::ConstPtr& context, std::string name)
  {
    try
    {
  ...
  template <typename T>
  [[nodiscard]] std::optional<T> try_get(const Context::ConstPtr& context)
  {
    return try_get<T>(context, typeid(T).name());
  }

  template <typename T, typename Callable>
  [[nodiscard]] auto get_or(const Context::ConstPtr& context, std::string name, const Callable& default_value)
  ...
  template <typename T, typename Callable>
  [[nodiscard]] auto get_or(const Context::ConstPtr& context, const Callable& default_value)
    -> std::enable_if_t<std::is_same_v<T, std::remove_reference_t<decltype(default_value())>>, T>
  {
